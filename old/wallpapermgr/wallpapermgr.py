...
class WallpaperArchiveBase(object):
    ...
    def get_archive(self, archive_name=None):
        ...
        Attributes:
            :py:attr`archive_path`
            :py:attr`archive_info`
            :py:attr`archive_name`
        ...
        if 'archives' not in config:
            raise RuntimeError(
                'config has no section "archives": "%s"' % self.configfile)
        ...
        if archive_name in config['archives']:
            ...
            raise ConfigKeyMissing(
                'No archive named "%s" in user config' % archive_name)
    ...
    def _determine_archive(self):
        ...
        for archive_name in config['archives']:
            ...
            if conditions_satisfied == len(conditions):
                logger.info(
                    'All conditions matched for archive: {archive_name}'.format(**loc()))
                return archive_name

        # if no archive had all of it's conditions satified,
        # use the default
        logger.info(
            "No Archive satisfied all it's conditions. Using Default Archive: {default_archive}".format(**loc()))
    ...
    def _eval_condition(self, condition):
        ...
        for conditionpath in conditionpaths:
            if condition in os.listdir(conditionpath):
                ...
                return_raw = subprocess.check_output(
    ...
    def get_wallsequence(self, reload_tarfiles=False):
        ...
        logger.debug(
            'last wallpaper({last_fileno}): "{last_file}"'.format(**loc()))
    ...
    def _get_saved_wallsequence(self):
        ...
        """
        logging.debug(
            'Datafile is up-to-date, getting sequence and last-displayed file')
    ...
    def _refresh_wallsequences(self, datafile):
        ...
        Args:
            datafile (str): ``(ex: '/path/to/archive.tar' )``
                the archive we are setting a sequence of walls for.
        ...
        for archive_name in config['archives']:
            ...
            if os.path.isfile(archive_path):
                logger.info(
                    'Reading files in Archive: "{archive_path}"'.format(**loc()))
        ...
        if not data:
            raise RuntimeError(
                'Data variable is empty. No wallpapers to display')
    ...
    def _refresh_wallsequence(self, data, archive_path):
        ...
        Args:
            archive_path (str): ``(ex: '/path/to/file.tar')``
                path to a tar archive containing a set of images.
    ...
    def _parse_returnjson_raw(self, return_raw):
        ...
        if 'return' not in return_json:
            raise excepts.InvalidJSONReturn(
                'Returned JSON is missing "return" key')
...
class DisplayWallpaper(WallpaperArchiveBase):
    ...
    def _validate_args(self):
        ...
        if index != None and not isinstance(index, int):
            raise TypeError(
                'index must be a positive integer (ex: 10): {index}'.format(**loc()))
        ...
        if offset != None and not isinstance(offset, int):
            raise TypeError(
                'offset must be a positive or negative integer (1/-1): {offset}'.format(**loc()))
    ...
    def _get_next_fileno(self):
        ...
        if index != None:
            ...
            if (offset + last_fileno) < len(wall_sequence):
                ...
                next_fileno = ((offset + last_fileno) %
                               wall_sequence) - len(wall_sequence)
            else:
                next_fileno = len(wall_sequence) - \
                    ((offset + last_fileno) % wall_sequence)
            ...
            if hasattr(archive_info['apply_args'], '__iter__'):
                for arg in archive_info['apply_args']:
                    if isinstance(arg, basestring):
                        ...
                        raise TypeError(
                            'Invalid Datatype for argument: %s' % arg)
    ...
    def _run_apply_method(self, index, filepath, args, kwds, specialvars):
        ...
        logger.info(
            'Displaying Wallpaper ({index}): "{filepath}"'.format(**loc()))
        archive_path = self.archive_path

        # determine apply-method
        scriptdir = os.path.dirname(os.path.realpath(__file__))
        apply_method = self.archive_info['apply_method']
        apply_script = '{scriptdir}/apply_methods/{apply_method}'.format(
            **loc())

        if not os.path.isfile(apply_script):
            raise IOError(
                'Expected script at: "{apply_script}"'.format(**loc()))
        ...
        pipe = subprocess.Popen(apply_script, stdin=subprocess.PIPE,
...
class Archive(WallpaperArchiveBase):
    ...
    def git_operation(self, archive_name, operation):
        ...
        self.get_saveddata()
        self.get_archive(archive_name)
        git_info = gitoperations.Git().git_configured(
            self.config, self.data, archive_name)
...
class CLI_Interface():
    ...
    def subparser_shortcmds(self):
        parser = self.subparsers.add_parser(
            'next',    help='Display next wallpaper\n (short for `wallmgr display --next`)')
        parser = self.subparsers.add_parser(
            'prev',    help='Display previous wallpaper\n (short for `wallmgr display --prev`)')
        parser = self.subparsers.add_parser(
            'shuffle', help='Shuffle existing order of wallpapers\n (short for `wallmgr data --shuffle-order`)')
        parser = self.subparsers.add_parser(
            'pull',    help='Pull latest wallpapers from git repository')
        parser = self.subparsers.add_parser(
            'push',    help='Push wallpapers to git remote')
        parser = self.subparsers.add_parser(
            'ls',      help='List configured archives')

    def subparser_display(self):
        parser = self.subparsers.add_parser(
            'display', help='Commands related to Displaying wallpapers\n (see `wallmgr display --help`)')
    ...
    def subparser_data(self):
        parser = self.subparsers.add_parser(
            'data', help="Manage the program's internal datafile, image-order, etc\n (see `wallmgr data --help`)")
    ...
    def subparser_archive(self):
        parser = self.subparsers.add_parser(
            'archive', help="Manage archives containing wallpapers (append,create,delete,...)\n (see `wallmgr archive --help`)")
    ...
    def parse(self):
        ...
        # quick-commands
        if args.subparser_name == 'next':
            DisplayWallpaper(offset=1)
        elif args.subparser_name == 'prev':
            DisplayWallpaper(offset=-1)
        elif args.subparser_name == 'shuffle':
            DataFile(shuffle=True)
        elif args.subparser_name == 'push':
            DisplayWallpaper(offset=1)
        elif args.subparser_name == 'pull':
            DisplayWallpaper(offset=-1)
    ...
    def _parse_display(self, args):
        ...
        def error_singlearg():
            print(
                "use of (--prev, --next, --offset, --index)   cannot be used with any other argument")
            sys.exit(1)
